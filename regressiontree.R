...
# Random Sampling for training set
...
CartFullModel = rpart(formula = Annual_Sales ~ . , data = Mer_SalesTrainUncapped[,-1], method = "anova")  #We dont need merchant id, so -1, method = anova is for regression tree-numerical
CartFullModel
summary(object = CartFullModel)
summary(Mer_SalesTestUncapped[,'Annual_Sales'])
names(Mer_SalesTrainUncapped)
# Plot the Regression Tree
rpart.plot(x = CartFullModel, type = 2,fallen.leaves =T, cex = 1, nn=T, tweak=0.6)
