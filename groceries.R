...
library(arules)  #install first for the three factors:SUPPORT, CONFIDENCE, LIFT
library(arulesViz) #install first for visualisation devices like graphing
library(datasets)  # no need to install, just load it
data('Groceries')

str(Groceries)
Groceries
inspect(Groceries[1:5])  #view first 5 transactions

LIST(Groceries[1:6])  #another view

#Lets Apply Apriori Algorithm
frequentItems <- eclat (Groceries, parameter = list(supp = 0.005, minlen= 1, maxlen = 5))    #Used to filter items by support value
frequentItems
...
?eclat
itemFrequencyPlot (Groceries,topN = 15,type="absolute") #Total number of transactions involving this
itemFrequencyPlot(Groceries, topN = 10, type='relative') #Precentage of total transactions
abline(h=0.2)
...
options (digits=2)
inspect (rules[1:5])
rulesc <- sort (rules, by="confidence", decreasing=TRUE) #Filter by onfidence descending order
inspect(rulesc[1:5])
rulesl <- sort (rules, by="lift", decreasing=TRUE) #Filter by lift descending
inspect (rulesl[1:5])
